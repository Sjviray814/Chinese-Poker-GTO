import random, time
import solver as S
from pivotal_heuristic import heuristic_action_pivotal
...
def run_matchup(n, base_seed=0):
    pivotal_wins = old_wins = ties = 0
    t0 = time.time()
    for i in range(n):
        seed = base_seed + i
        if i % 2 == 0:
            wins = play_game(heuristic_action_pivotal, S.heuristic_action, seed)
            pw, ow = wins[0], wins[1]
        else:
            wins = play_game(S.heuristic_action, heuristic_action_pivotal, seed)
            pw, ow = wins[1], wins[0]
        if pw > ow: pivotal_wins += 1
        elif ow > pw: old_wins += 1
        else: ties += 1
    elapsed = time.time()-t0
    print(f"PIVOTAL-AWARE vs CURRENT: {pivotal_wins}-{old_wins}-{ties} over {n} games ({100*pivotal_wins/n:.1f}%)  [{elapsed:.0f}s, {n/elapsed:.0f} games/sec]")
