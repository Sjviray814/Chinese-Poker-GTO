import random, time
import solver as S
from hand_aware_test import heuristic_action_hand_aware
...
if __name__ == '__main__':
    ...
    for i in range(n):
        if i % 2 == 0:
            w = play_mcts_game(heuristic_action_hand_aware, S.heuristic_action, 950000+i)
            nw, ow = w[0], w[1]
        else:
            w = play_mcts_game(S.heuristic_action, heuristic_action_hand_aware, 950000+i)
            nw, ow = w[1], w[0]
        if nw > ow: new_wins += 1
        elif ow > nw: old_wins += 1
        print(f'  game {i+1}/{n}: running record {new_wins}-{old_wins} ({time.time()-t0:.0f}s)')
    print(f'MCTS-LEVEL: hand-aware {new_wins} - original {old_wins} over {n} games')
