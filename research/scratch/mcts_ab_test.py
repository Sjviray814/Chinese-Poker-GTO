...
def mcts_decide(hand, own_table, opp_table, own_burned, own_burn_card, opp_burned, heuristic_fn, time_budget, rng):
    """Run solve() with a specific heuristic temporarily active inside
    rollout, for a fair A/B at the SEARCH level (not just raw heuristic)."""
...
if __name__ == '__main__':
    ...
    for i in range(n):
        if i % 2 == 0:
            w = play_mcts_game(S.heuristic_action, S.heuristic_action_old, 300000+i)
            nw, ow = w[0], w[1]
        else:
            w = play_mcts_game(S.heuristic_action_old, S.heuristic_action, 300000+i)
            nw, ow = w[1], w[0]
        if nw > ow: new_wins += 1
        elif ow > nw: old_wins += 1
        else: ties += 1
        print(f'  game {i+1}/{n} done, running record: {new_wins}-{old_wins}-{ties}  ({time.time()-t0:.0f}s elapsed)')
    print(f'FULL MCTS-LEVEL: new-heuristic-rollout {new_wins} - old-heuristic-rollout {old_wins} - ties {ties}  over {n} games')
