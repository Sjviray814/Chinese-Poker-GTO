...
import solver as S
from microsim_eval import mcts_search_microsim
...
def microsim_strategy(time_budget, iters_per_det, seed_base, leaf_samples=8):
    turn = {'n': 0}
    def strat(state, player, burn_cards):
        turn['n'] += 1
        ranked, dets = solve_generic(mcts_search_microsim,
...
if __name__ == '__main__':
    ...
    TIME_BUDGET = 0.6
    ITERS = 60
    m_wins = r_wins = ties = 0
    margins = []
    t0 = time.time()
    for g in range(N_GAMES):
        m_strat = microsim_strategy(TIME_BUDGET, ITERS, seed_base=9000+g, leaf_samples=8)
        r_strat = rollout_strategy(TIME_BUDGET, ITERS, seed_base=10000+g)
        if g % 2 == 0:
            wins = play_game(m_strat, r_strat, seed=g)
            mw, rw = wins[0], wins[1]
        else:
            wins = play_game(r_strat, m_strat, seed=g)
            mw, rw = wins[1], wins[0]
        if mw > rw: m_wins += 1
        elif rw > mw: r_wins += 1
        else: ties += 1
        margins.append(mw - rw)
        print(f"game {g:2d}: MICROSIM {mw} - {rw} ROLLOUT   [{time.time()-t0:.0f}s elapsed]")
    print()
    print(f"MICROSIM vs ROLLOUT record: {m_wins}-{r_wins}-{ties} over {N_GAMES} games (same {TIME_BUDGET}s/move)")
    print(f"Average column-margin for MICROSIM: {sum(margins)/len(margins):+.2f}")
